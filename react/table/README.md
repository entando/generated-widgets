...
1. Downlaod and install Keycloak standalone server (this widget has been tested with Keycloak 7.0.1) from https://www.keycloak.org/downloads.html
...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/conference/tableWidget/>):
   1. Run `npm i`
...
- **conference.table.error** (fired when an error occurs)
- **conference.table.select** (fired when a table row is selected)
